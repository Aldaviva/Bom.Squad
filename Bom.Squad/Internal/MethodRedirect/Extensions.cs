...
/// <summary>
/// Source: https://github.com/spinico/MethodRedirect/blob/1e2a681c22dc3781cfd36a8f683c3b808456f429/Sources/Extensions.cs
/// </summary>
internal static class Extensions {

    public static MethodOperation RedirectTo<T>(this MethodInfo origin, Func<T> target, bool verbose = false)
        => origin.RedirectTo(target.Method, verbose);

    public static MethodOperation RedirectTo<T, TR>(this MethodInfo origin, Func<T, TR> target, bool verbose = false)
        => origin.RedirectTo(target.Method, verbose);
    ...
    public static MethodOperation RedirectTo(this MethodInfo origin, MethodInfo target, bool verbose = false) {
        ...
        if (verbose) {
            Debug.WriteLine("\nPlatform      : {0}", new object[] { IntPtr.Size == 4 ? "x86" : "x64" });
            Debug.WriteLine("IntPtr.Size   : {0}", IntPtr.Size);

            Debug.WriteLine("\nFrom origin method    : {0}", new object[] { origin.Name });
            OutputMethodDetails(origin, ori);

            Debug.WriteLine("\nTo target method      : {0}", new object[] { target.Name });
    ...
    private static void OutputMethodDetails(MethodInfo mi, IntPtr address) {
        ...
        Debug.WriteLine("Method is virtual     : {0}", mi.IsVirtual);
        Debug.WriteLine("MethodDescriptor (MD) : \t{0}", new object[] { md.ToString("x").PadLeft(8, '0') });
        Debug.WriteLine("MethodTable (MT)      : \t{0}", new object[] { mt.ToString("x").PadLeft(8, '0') });
        Debug.WriteLine("Offset (MT - MD)      : \t{0}", new object[] { offset.ToString("x").PadLeft(8, '0') });
        ...
        if (mi.IsVirtual) {
            ...
            Debug.WriteLine("MethodTable offset    : \t{0}", new object[] { offset.ToString("x").PadLeft(8, '0') });
            Debug.WriteLine("First method slot     : {0}\t{1}",
            ...
            Debug.WriteLine("\nMethodDesc data       : {0}", new object[] { Marshal.ReadInt64(md).ToString("x").PadLeft(8, '0') });
            Debug.WriteLine("Right-shift 32 bits   : {0}", new object[] { shift.ToString("x").PadLeft(8, '0') });
            Debug.WriteLine("Mask                  : {0}", new object[] { mask.ToString("x").PadLeft(8, '0') });
            Debug.WriteLine("Method slot number    : {0}", slot);

            Debug.WriteLine("\nMethodDesc data       : {0}", new object[] { Convert.ToString(Marshal.ReadInt64(md), 2).PadLeft(32, '0') });
            Debug.WriteLine("Right-shift 32 bits   : {0}", new object[] { Convert.ToString(shift, 2).PadLeft(32, '0') });
            Debug.WriteLine("Mask                  : {0}", new object[] { Convert.ToString(mask, 2).PadLeft(32, '0') });
            Debug.WriteLine("Method slot number    : {0}\n", new object[] { Convert.ToString(slot, 2).PadLeft(32, '0') });

            Debug.WriteLine("Relative offset       : {0}", new object[] { (IntPtr.Size * slot).ToString("x").PadLeft(8, '0') });
        }

        offset = (int) ((long) address - (long) mt);

        Debug.WriteLine("Jitted method (JM)    : {0}\t{1}", address.ToString("x").PadLeft(8, '0'),
            Marshal.ReadIntPtr(address).ToString("x").PadLeft(8, '0'));

        Debug.WriteLine("Offset (JM - MT)      : {0}", offset);
    }
    ...
    private static IntPtr GetMethodAddress(MethodInfo mi) {
        const ushort slotNumberMask = 0xffff; // 2 bytes mask
        const int    mtOffset32Bit  = 0x28;   // 40 bytes offset
        const int    mtOffset64Bit  = 0x40;   // 64 bytes offset
    ...
    private static MethodRedirection Redirect(IntPtr ori, IntPtr tar, bool verbose) {
        ...
        MethodRedirection token = new(ori);

        if (verbose) {
            Debug.WriteLine("\nRedirect...");
            Debug.WriteLine("From {0} [{1}] => To {2} [{3}]",
        ...
        Marshal.Copy(new[] { Marshal.ReadIntPtr(tar) }, 0, ori, 1);
